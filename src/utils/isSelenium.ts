...
export default function isSelenium(): Promise<boolean> {
  return new Promise(resolve => {
    ...
    for (const documentDetectionKey in documentDetectionKeys) {
      ...
      if (documentDetectionKeyValue)
        if ((window['document'] as any)[documentDetectionKeyValue] || documentDetectionKeyValue in document) {
          return resolve(true);
        }
    }
    for (const documentKey in window['document']) {
      if (documentKey)
        if (documentKey.match(/\$[a-z]dc_/) && (window['document'] as any)[documentKey]['cache_']) {
          return resolve(true);
        }
    }
    if (
