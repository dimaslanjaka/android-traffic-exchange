 ...
 */
export default function fillFixedPosition(currentSlot: ReturnType<typeof getCurrentSlot>, options: AdsenseOption) {
  if (!currentSlot) currentSlot = getCurrentSlot(options);
    ...
    for (let i = 0; i < fixedPlacement.length; i++) {
      ...
      if (currentSlot) {
        ...
        if (currentSlot.ads.length > 0) {
          const newAttr = currentSlot.ads.shift();
        ...
        if (attr) {
          ...
          const ins = createIns(attr, options);
          if (ins && placement) {
            // console.log('fill', attr);
            // replaceWith(ins, placement);
            placement.appendChild(ins);
          } else if (!placement) {
            console.error('placement undefined');
          } else {
            console.error('ins undefined');
          }
        }
      }
    }
